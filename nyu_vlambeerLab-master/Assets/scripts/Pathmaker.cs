...
public class Pathmaker : MonoBehaviour {
	...
	public static int globalTileMax = 100;
	public static int offsetSpacing = 5;

	public static int existTime = 200;
    ...
    void Update()
	{
		...
		if (Pathmaker.existTime >= 0)
		{
			for (int x = 0; x < Pathmaker.globalTileMax; x += Pathmaker.offsetSpacing)
			{
				for (int z = 0; z < Pathmaker.globalTileMax; z += Pathmaker.offsetSpacing)
				{
					//Color colors = Random.ColorHSV(0f, 0.5f, 0.5f, 1f);
					//ApplyMaterial(colors, 0);
					print("x:" + x);
					print("z:" + z);
					...
					Vector3 rotation = new Vector3(Random.Range(-0.5f, 0.5f), 0f, Random.Range(-0.5f, 0.5f));
					Vector3 scale = Vector3.one * Random.Range(0.5f, 1.5f);

					GameObject newElement = Instantiate(element.prefabs);
	...
	//void ApplyMaterial(Color color, int tatgetMaterialIndex)
	//{
	...
	//{
		...
		//if (Pathmaker.globalTilecount < 500 && Pathmaker.existTime >= 0)
		//{
		//	float randomNumber = Random.Range(0f, 1f);

		//	Pathmaker.existTime = Random.Range(200, 2000);
		...
		//		this.transform.Rotate(0, -90, 0);
		//	}
		//	else if (randomNumber >= 0.80f && randomNumber <= 1f)
		//	{
		//		GameObject newPMS = Instantiate(pathmakerSphere, this.transform.position, Quaternion.Euler(0, 0, 0));
		//	}

			//GameObject newPath = Instantiate(floor, this.transform.position, Quaternion.Euler(0, 0, 0));
		//}
		...
		//	//print("globalTileCount" + Pathmaker.globalTileCount);


		//}
		//else if (Pathmaker.globalTilecount >= 500 || Pathmaker.existTime < 0)
		//{
		//	//print(counter + "finished");
		//	//print(Pathmaker.globalTileCount + "finished");
			//Destroy(this);
